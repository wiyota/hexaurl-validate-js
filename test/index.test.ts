...
describe('HexaURL Validation', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should provide descriptive error messages', () => {
      let errorThrown = false;
      try {
        validate('test--test', createConfig({
          delimiter: createDelimiterRules({ allowConsecutiveHyphens: false })
        }));
      } catch (error) {
        errorThrown = true;
  ...
  describe('Byte Size Handling', () => {
    it('should respect byte size constraints', () => {
      ...
      expect(() => validate('abcdefghijklm', undefined, smallByteSize)).toThrow(HexaUrlError);
      expect(() => validate('abcdefghij', undefined, smallByteSize)).not.toThrow();
      
      expect(() => validate('a'.repeat(45), undefined, largeByteSize)).toThrow(HexaUrlError);
      expect(() => validate('a'.repeat(40), undefined, largeByteSize)).not.toThrow();
    });
  });
});
